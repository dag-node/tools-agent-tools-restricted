...
#
# The "shipped set" is the core, each STABLE group, and each integration's layout module, derived by shipped-modules.sh
# from the group registry and the integration manifests -- the same list the RPM build compiles. `build` and `install`
# stage it compiled under /usr/share/selinux/packages/ai-tools, where the installed ai-tools-admin loads a group
# from with no checkout and no toolchain, as it does on an RPM host.
#
# The optional policy groups are all DISABLED by default (the core alone covers repo-only work) and are declared once,
# in selinux-groups.lib.sh -- name, description, why it is off, and the stability that decides whether it is
# on the shipped set. This script reads that registry, as ai-tools-admin does, so the two cannot disagree
# on which groups exist.
...
# Policy source, the script naming the shipped set, and the modules compiled from the source live under policy/;
# the build (`make -C`) and every .te/.fc/.pp reference resolve there. install-selinux.sh, README.md, and ../src stay
# at DIR.
readonly POLICY_DIR="${DIR}/policy"
readonly MODULE="ai_tools"

# refuse_early <code> <line>... -- the refusals that answer before the styled emitters are defined: a library that will
# not load, and the root guard. Renders what plain mode renders -- the code on its own leading line, then each caller
# line whole -- and exits 1. The matcher is the library's own anchored form (tests/unit/msg.sh holds every inline copy
# to it).
refuse_early() {
    local code=""
...
# refuse_unsourced <lib> -- one situation for every library this script requires, whichever of the two locations it was
# looked for in.
...
# Optional policy-group registry (names/descriptions/reasons + predicates), single-sourced so this authoring tool
# and the installed ai-tools-admin never disagree on the group set. REQUIRED -- the enable/disable/list actions
# and the install prompt all read it; a missing lib fails the run. Same source-tree-first, installed-second resolution
# as MSG_LIB.
...
# A deliberate twin of install.sh's non-root refusal (messaging.rule.md): the code is defined there and printed here,
# so an operator meets one token whichever installer refused.
...
# Sandbox clones live here and are labelled ai_tools_project_t by the STATIC rule in ai_tools.fc, so the per-project
# semanage loop skips them (a duplicate local rule would be redundant). A plain restorecon of this tree applies
# the static label.
readonly SANDBOX_PROJECTS="/var/opt/ai-tools/sandbox-projects"
# The user-owned ai-tools config dir (allowed-projects, secret-patterns). Labelled ai_tools_conf_t so the root helpers
# -- which run IN ai_tools_handback_t, inherited from the handback daemon -- can read the allowlist; without it their
# getattr is denied (config_home_t:file is dontaudit'd) and ownership handback silently no-ops. The narrow type is
# what keeps that grant off the rest of ~/.config. The confined session is granted the same type, which the 700/600
# modes then gate -- see the ai_tools_conf_t block in ai_tools.te. Applied via semanage (dynamic home path), not
# ai_tools.fc (fixed paths).
#
# The label belongs to an ACCOUNT, so the sweep covers every operator this host has (_operator_conf_dirs). It repairs
# an account enrolled while the policy was absent: `ai-tools-admin operators add` registers the rule for each account it
# enrols, and there is no type to assign until the module is loaded.
readonly CONF_TAIL=".config/ai-tools"
# Root-helper operation logs. Labelled ai_tools_log_t (static rule in ai_tools.fc) so the helpers that run
# IN ai_tools_handback_t (chown, setgid, launcher-symlink) may append under enforcing. A plain restorecon applies
# the label; created by install.sh.
...
# from PID1's CACHED file_contexts DB. A policy update that adds/changes the /run/ai-tools fcontext (ai_tools_run_t)
# leaves that cache stale, so the dir -- and the handback.sock inside it -- are recreated var_run_t, which ai_tools_t
# may not write (ai_tools.te grants only ai_tools_run_t:sock_file write), breaking every hook handback.
# _relabel_runtime() repairs this; a fresh boot reads the current fcontext correctly.
readonly RUN_DIR="/run/ai-tools"

# Styled output mirroring install.sh so the two installers read the same. Colours only on a TTY. stdout carries status;
# warnings and the group prompt go to stderr (warn/logx/sayx) so they never contaminate stdout.
if [[ -t 1 ]]; then
...
# indented: shift a subordinate command's own output one level in, so it reads as a block beneath the `log` line
# that announced it rather than breaking the transcript's left margin. Used on a pipeline, which `pipefail` (set
# at the top of this file) leaves carrying the command's exit status; the caller merges both streams into it,
# so a failure message is indented with the rest and still reaches the install transcript.
indented() { sed 's/^/    /'; }
# logx/sayx: stderr variants -- safe inside subshells, and used for the group prompt, which must not contaminate stdout.
logx()    { printf '  %s+%s %s\n' "${C_DIM}" "${C_RST}" "$*" >&2; }

# _list <item...>: render a set as [a, b] -- a name list reads as one value that way, where space-separated names blur
# into the prose around them and hide how many there are. Joined by hand rather than through IFS: "$*" uses only
# the FIRST character of IFS, so a ", " separator silently loses its space.
_list() {
    local joined="" item
...
# _group_cmd <enable|disable> [name]: the command an operator on THIS host should run to manage a policy group.
# ai-tools-admin is the shipped entry point and is on PATH once the package is installed, so prefer it; a source
# checkout with no install yet falls back to this script's own path. The two front doors spell the action differently --
# the shipped one follows the command grammar (.claude/rules/cli-grammar.rule.md) while this developer-only script keeps
...
# require_devel <pp>: exit with install guidance unless the devel toolchain is present. Reached by every action
# that compiles: a checkout carries no compiled module, so the first install builds each one, and a rebuild
# after editing a .te/.fc builds it again.
require_devel() {
...
# _shipped_modules: print the shipped set, one module name per line -- the derivation in shipped-modules.sh, read
# from this checkout's registry and manifests. A derivation that fails aborts the run: staging a guessed set would leave
# ai-tools-admin a package directory that does not match what the registry calls stable.
_shipped_modules() {
...
# a current group may require a type the old core did not declare -- and before a group is enabled or disabled, so every
# path that loads policy from this checkout migrates the
# host.
...
# _load_layout_modules: load the layout module of every installed integration that declares one (selinux_layout_module
# in its manifest, read through providers.lib.sh with its trust rules). A layout module types an integration's
# build-output directories and does not add any permission, so it is not a group an operator enables: it loads whenever
...
# out -> "ENFORCING". Otherwise -> "PERMISSIVE (<dom> ...)" listing the still-permissive domains.
_mode_label() {
    local doms
...
# _check_permissive_alignment: after semodule -i, verify that no stale semanage-managed permissive_<domain> module is
# keeping a domain permissive despite the compiled .te expecting it to be enforcing.  Warns and offers to remove
# the stale module interactively; prints the fix command otherwise.
_check_permissive_alignment() {
    # Domains the compiled .te expects permissive (non-commented permissive lines). A no-match grep exits 1,
    # which pipefail propagates to the assignment and `set -e` would abort on -- the normal ENFORCING case has zero
    # permissive lines here, so tolerate an empty result (the `-z` checks are the intended empty-path).
...
#
# Prints everything to stderr so it doesn't contaminate stdout. Populates SELECTED_GROUPS (not-loaded groups to enable)
# and RECOMPILE_GROUPS (loaded groups to rebuild + reload).
########################################
SELECTED_GROUPS=()
RECOMPILE_GROUPS=()
...
prompt_groups() {
    ...
    # the experimental subset instead of the whole set.
    ...
    # State what is already loaded before the gate: the skip path only ADDS or REBUILDS modules, so a group an earlier
    # install enabled survives a skip -- silence would read as if it might
    # not.
    if (( ${#loaded_groups[@]} )); then
        sayx ""
        logx "already loaded and kept: ${C_BOLD}$(_list "${loaded_groups[@]}")${C_RST}"
        # Two calls, not one with a line-continuation: sayx joins its args through "$*", which under this script's
        # IFS=$'\n\t' glues them with a NEWLINE -- so the command would land unindented on its own line. Keep the note
        # and its (indented) command as separate lines.
    ...
    # Stable groups are offered first, experimental ones after: the stable set is what an operator enables without
    # an audit, so it is what the prompt leads with, and the registry's own order (which groups an integration's
    # declaration) is kept within each half. A group an installed integration declares (selinux_groups in its manifest)
...
# for the whole run: the install action prompts between its loads, and a lock held across a prompt makes the watcher's
# run wait out AI_TOOLS_RELABEL_LOCK_WAIT and then proceed unserialized. An untaken lock is reported once
# and the command runs anyway, the library's own fail-soft.
...
# _labels_apply / _labels_drop: the fcontext sections, one _locked call each. Apply registers and verifies every enabled
# agent's rules, the enrolled operators' config rules, and each registered project's rule; drop removes them
# in the reverse order, while the module still declares their
# types.
...
_labels_drop() {
    ...
    _unlabel_conf
    # The agents' path rules are local fcontexts naming types the module unload removes. Dropped for EVERY installed
    # agent manifest, not just the enabled ones: a disabled agent may still hold a rule from when it was on.
...
verify_agent_labels() {
    ...
    if [[ -n "${report}" ]]; then
        # Pin IFS for this read: the script runs under the strict-mode IFS=$'\n\t', and the report's fields are
        # SPACE-separated, so an inherited IFS puts the whole line in ${verdict} and every case arm misses -- including
        # `bad`, which is what sets the flag that aborts the install when an entrypoint did not take ai_tools_exec_t.
        # The guard against launching unconfined depends on this splitting correctly.
        while IFS=$' \t\n' read -r verdict subject detail wanted; do
            case "${verdict}" in
                ...
                # The declared rule does not cover the entrypoint the agent's launcher actually resolves to, so no rule
                # this sweep applies can label it and the session would be refused. Counted as `bad`: the install must
                # not report a confined host.
                stale) bad=1
                ...
                # `failed` prints, naming the agent those lines omit.
                agent)
                    if [[ "${detail}" == failed ]]; then
                        warn MSG-E6C4 "labelling did not complete for ${subject} -- see its lines above"
                    fi ;;
                # A verdict this renderer does not know is REPORTED, not dropped. Silently ignoring one turns
                # a labelling result into no output at all, which reads as "no change" for the one path whose label
                # decides whether a session is confined -- and leaves the operator no detail to diagnose from.
    ...
    # A path that restorecon left mislabelled is an unrecoverable gap (the module is loaded but the transition would not
    # fire, or the agent cannot write its state), so fail the install here rather than proceed to the optional groups
    # with a broken core. A missing path (toolchain not provisioned yet) stays a warning -- there is no entrypoint
    # to label.
    [[ "${bad}" -eq 0 ]] \
        || die MSG-U3S8 "an agent path is not correctly labelled (see above) -- the session would be refused, or run UNCONFINED"
    # Nothing labelled has two very different causes, and the bare message named neither. An EMPTY report means no
    # enabled agent was iterated at all -- the manifests resolved to no file -- which is a configuration problem:
    # the entrypoint keeps whatever type it has, and a launch fail-closes at ai-tools-run's transition preflight.
    ...
    # Printed while the install is still running, so it states WHEN it applies: an operator who reads "exit
    # and relaunch" mid-install has no session to relaunch yet.
...
# for_each_project <fn>: call <fn> once with each allowlisted project directory, skipping blank/comment/'!'-exclusion
# lines and sandbox clones (labelled statically by ai_tools.fc). No-op when the allowlist is absent.
for_each_project() {
...
# _label_one/_unlabel_one: thin wrappers over the shared lib so this sweep and the ai-tools-relabel helper share one
# implementation. Non-zero is swallowed (warn, don't die) so one bad project never aborts a whole relabel. _unlabel_one
# already restorecons via the lib; the remove action's later _restore_one pass is a harmless belt-and-suspenders.
# Re-asserts the label on every registered project each run. The relabel is idempotent (restorecon writes only a file
...
# _operator_conf_dirs: print the ai-tools config directory of every account this host treats as an operator, one
# per line, deduplicated in first-seen order. That set is the OPERATORS list in operator.conf plus the invoking user,
# who is an operator by having run this and who on a first install is absent from the list, operator.conf being written
# by `ai-tools-admin operators add`. So an unenrolled host still labels the config of the account installing the policy.
_operator_conf_dirs() {
    local name home
    {
        printf '%s\n' "${PROJECTS_USER}"
...
# Label / unlabel every operator's ~/.config/ai-tools as ai_tools_conf_t (see CONF_TAIL comment). The rule registration
# and the restorecon live in relabel.lib.sh, so this sweep and the per-account registration in ai-tools-admin apply one
# implementation.
_label_conf() {
    local dir status
    while IFS= read -r dir; do
        ...
        case "${status}" in
            ...
            # ai_tools_conf_t must already exist in the LOADED policy for semanage to accept it. 'relabel' never loads
...
# trees is enough because they live on persistent filesystems, but the handback runtime dir is tmpfs and recreated
# by systemd from PID1's cached label DB, so three steps are needed: (1) daemon-reexec re-execs PID1 so it reloads
# the now-current file_contexts (the root cause of the stale var_run_t label); (2) restart the socket
# so RuntimeDirectory is recreated with the refreshed context; (3) restorecon the live path as a belt-and-suspenders
# for the already-running dir. Each step is best-effort: if the socket unit is absent (handback not installed) the whole
# thing no-ops. A hook firing during the brief socket restart simply no-ops via its `|| true` and is recovered
# by the next sweep. Callers run _relabel_helpers first: the restart makes systemd derive the listening socket's context
# from the daemon binary's on-disk label, so the daemon must already carry ai_tools_handback_exec_t when the socket
# rebinds.
_relabel_runtime() {
...
# (/usr/local/libexec/ai-tools/ai-tools-handback, ai_tools.fc). Without this the daemon keeps a generic label,
# the init_t -> ai_tools_handback_t transition never fires, the per-connection handler runs in unconfined_service_t,
# and ai_tools_t's connectto (granted only to ai_tools_handback_t) is denied -- every hook handback fails with EACCES.
# The sibling root helpers and the /usr/local/bin client are bin_t (no special label). restorecon is idempotent
# and no-ops when handback is not installed. Runs before _relabel_runtime's socket restart, which reads this label (see
# there).
...
case "${ACTION}" in
  ...
  install)
    section "Core module"
    # ensure_pp compiles the core on a fresh checkout and, with the toolchain present, lets make decide whether
    # an earlier build still matches the source -- so an edited .te/.fc is loaded by a plain re-run, attended or not.
    ensure_pp "${MODULE}.pp"
    ...
    _replace_former_group_modules
    _load_layout_modules
    # The shipped set, compiled and staged where the installed ai-tools-admin loads a stable group from; each module
    # goes through ensure_pp, so an edited source is rebuilt with the core.
    ...
    # Label the handback daemon first: the socket restart in _relabel_runtime derives the listener's context
    # from the daemon binary's on-disk label at bind time.
    _relabel_helpers
    ...
    # Core is loaded and labelled -- a clear checkpoint before the optional groups. Reaching here means the preceding
    # steps succeeded (a hard failure aborts under `set -e`; a mislabelled path dies in verify_agent_labels),
    # so the optional section is purely additive.
    ...
    if (( ${#SELECTED_GROUPS[@]} || ${#RECOMPILE_GROUPS[@]} )); then
        ...
        # Reload a loaded group from its current source: build_pp requires the toolchain, so a reload asked
    ...
    # Report what THIS run changed separately from the full loaded set: a group an earlier install enabled is kept
    # unless the operator re-selected it here, so naming only the changes would read as if a kept group had become
    # disabled. _list, not "${arr[*]}": this script runs IFS=$'\n\t', so [*] joins the names with a NEWLINE and each
    # lands on its own unindented line. _list renders them as [a, b].
    ...
    # Layout modules are reported apart from the groups: they load with an integration, not by an answer to this prompt,
    # and a missing one is why fresh build output types ai_tools_project_t.
  ...
  build)
    # Compile the shipped set from source and stage it under the package directory, loading nothing: the step install.sh
  ...
  enable-group)
    ...
    _locked semodule -i "${POLICY_DIR}/ai_tools_${name}.pp"
    ok "group '${name}' enabled"
    # A group may ship file contexts of its own (dotnet maps a clone's build output), so the labels are re-applied
    # after the load: the project sweep re-asserts each project's rules, and the clone restorecon picks up any static
    # rule the group added.
  ...
  disable-group)
    ...
    if ai_tools_selinux_group_loaded "${name}"; then
        ...
        # for it. Every type the groups name is declared in the base, so a per-project rule outlives the group
        # unchanged.
