...
#
# The one derivation of the shipped set: the core (ai_tools), ai_tools_<group> for each STABLE group
# in selinux-groups.lib.sh, and the layout module each integration manifest declares (selinux_layout_module,
# ai-tools-providers(5)). The spec's %build compiles this list against the building distribution's policy headers
# and %install stages it; install.sh compiles and stages the same list from a checkout (through install-selinux.sh
# build); the tests hold the built package and the installed host to it. No module name is spelled anywhere else,
# so promoting a group edits the registry and adding a layout module edits a manifest.
#
# Lives beside the policy sources and carries their licence: it names what the policy build compiles, which makes it
# a script controlling compilation and so part of the corresponding source of every module the ai-tools-selinux package
# conveys (GPLv2 s.3).
#
