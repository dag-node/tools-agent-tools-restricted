...
# allowlist override. The CLI runs as the OPERATOR, prompts on /dev/tty, sudo's for each root step, and writes
...
# the allowlist and git safe.directory -- for that one project, removed again by the unclaim it then drives.
#
# It never adopts anything that already exists. Every path is one it created inside that fresh workspace, and removal
...
# policy, and no shared runtime state -- notably the updater's last-run stamp, which is READ and never written, because
# a check worth having is not worth breaking the reporting of a real update run for. That bounds what it can prove:
# a state the host does not already happen to be in is reported and skipped rather than manufactured, and the unit
# suites cover those from the other side, against fixtures they own. The one runtime state it does move is
# what a session moves on every turn: section 2c runs the ownership-handback hooks for real, as the sandbox account,
# against a fixture inside the claimed project, and the Stop sweep advances the shared sweep marker exactly
# as a session's would.
#
# REQUIREMENTS. Run as an enrolled operator (in OPERATORS and in ai-ops), NOT as root and NOT under sudo -- the claim,
...
note() { printf '        %s%s%s\n' "${C_D}" "$*" "${C_0}"; }
# sudo_why <what needs it> : say what the imminent "[sudo] password for ..." prompt is for. The CLI invokes sudo itself,
# once per root helper, so the prompt arrives with no context of its own and the operator would otherwise be typing
# a password blind. Printed immediately before the command that triggers it. sudo caches for a few minutes, so not every
# line is followed by a prompt -- which is why it names the step rather than promising one.
...
# Every path this script touches is one IT created, in a directory mktemp made fresh. Nothing pre-existing is reused,
# written into, or removed -- an existing path is a hard stop, not something to clean up, because a script that deletes
# what it did not create is one typo away from deleting the wrong thing. WORKSPACE is the single root; the removal rails
# refuse any path that is not inside it.
[[ -n "${HOME:-}" && -d "${HOME}" ]] || { echo "HOME is unset or not a directory" >&2; exit 2; }
# Named by the suite's fixture rule (tests/lib/harness.sh), group `manual`: the project the claim registers
# in the operator's allowlist sits under it, so an entry a kept or aborted run leaves reads as this suite's,
# and `tests/run.sh residue` finds the directory in the operator's home.
WORKSPACE="$(mktemp -d "${HOME}/.ai-tools-test-manual-workspace-XXXXXX")" \
    || { echo "could not create a workspace under ${HOME}" >&2; exit 2; }
# mktemp makes it 700, which BLOCKS the sandbox account -- and a claim under a blocking ancestor offers to grant
# a traverse-only ACL on it. Inside the workspace that is ours to give, so make it traversable up front and the offer
# never arises here. 711 is enter-but-not-list, the same shape the claim's own grant has.
chmod 711 "${WORKSPACE}"
readonly WORKSPACE
...
# safe_rm <path>: remove a path ONLY if every rail holds -- non-empty, absolute, strictly inside the workspace mktemp
# created for this run, and not a symlink (which rm -r would follow into by name if the path were swapped).
# Unprivileged, always: no step here may need root to undo, and a `sudo rm -rf` in a cleanup path is exactly the shape
# of accident this guards against. A path it refuses is reported for the operator to look at, never forced.
safe_rm() {
    local p="${1:-}"
    [[ -n "${p}" ]]                        || return 0
...
# No credential caching: each step asks when it needs to, under the line that says what it is for. A password entered
# up front buys fewer prompts at the cost of the one thing worth having here -- knowing which step you are authorizing.
...
# ── fixture ──────────────────────────────────────────────────────────────────────────────────
# The shape the operator's sketch describes: a project with a directory sealed 700 holding a 600 file, a secret-named
# file, ordinary content, and a git repo so the .git and safe.directory paths are exercised too.
#
# EVERY MODE IS PINNED, none inherited from the invoking shell's umask. Under a umask of 077 the project ROOT is born
# 700 -- owner-only -- and the claim then correctly seals the whole tree and does not grant access, so the ordinary half
# of this fixture (the half that must be opened up) silently stops existing and every later check has no path to act on.
# What the fixture is FOR is the contrast between a sealed path and an ordinary one, so both sides are stated outright.
section "Fixture"
...
# A sealed directory whose setgid belongs to a THIRD group -- the one piece of residue a claim keeps rather than strips,
# so the Review block has to say so. "Third" is relative to the claim's rule (neither the sandbox group nor the owner's
# own primary group), so one of the operator's own SECONDARY groups qualifies -- and chgrp to a group you are in does
# not need privilege, which keeps this script's own sudo use at zero. Without such a group the case is skipped, never
# faked with
# root.
THIRD_GROUP=""
for g in $(id -nG); do
...
# Did the claim actually open the ordinary half? Everything after this point acts on a granted tree -- the residue
# a seal strips, the group an unclaim hands back, the fingerprint --force looks for -- so a tree that was never granted
# produces a page of failures that all say the same thing and none of them about the code. Decide it once, here,
# and skip what depends on it.
TREE_CLAIMED=false
...
# Two sealed fixtures, because the strip's three arms are not all reachable on one path. A directory grouped
# to the sandbox account cannot carry a setgid bit an OPERATOR set: the kernel clears S_ISGID on chmod when the caller
# is not in the file's group, and the operator is deliberately not in that one. So the sandbox-grouped fixture exercises
# the ACL and group arms, and a second fixture -- grouped to the operator's OWN group, where chmod does keep setgid --
...
# wider (0670, say) would mean the ACL mask was recalculated upward -- a strip that grants, which is the failure
# `setfacl -n` exists to prevent.
...
# Runs while the project is still claimed, and does not need sudo at all: the pair edits one line of this operator's own
...
if [[ ! -r "${AL}" ]]; then
    ...
    # The consequence an operator has to be told about, since it is the one that costs data: while parked, the handback
    # no longer restores files written under this path.
...
# The live chain -- hook, handback socket, daemon, ai-tools-chown -- is the one guarantee the automated suite cannot
# drive: the daemon execs the helper with its own environment, so the helper reads the operator's REAL allowlist
# and the fixture must sit inside a project that allowlist names. The automated suite may not write that allowlist,
# and the checkout it runs from need not be a project, so the place a hook fixture can honestly live is here, inside
# the project section 1 claimed and section 3 unclaims. The fixture cannot be under /tmp either: pam_namespace
# polyinstantiation, where present, gives the hook's own session an empty /tmp instance.
#
# Each hook is run FOR REAL, as the sandbox account through `sudo -u`, with the JSON the harness would send it. That has
# two consequences this script's header otherwise rules out, both of them what a session does on every turn: the Stop
# sweep advances the shared sweep marker under /opt/ai-tools/.claude, and the tool-call record writes one line
# to the journal under the sandbox uid. Root is used for fixture ownership (to make a file born the agent's), to reach
# the sandbox account, and to read the journal back; the undo of every ownership change is the hand-back the hooks
# themselves perform, and the unclaim in section 3 hands back whatever a failed hook left.
#
# ONE sudo for the whole section. The agent's config directory is root:ai-tools with no world access and an operator
# stays OUT of the sandbox group by design, so even the presence probe needs root; and a host may prompt for a password
# on EVERY sudo, so the section is one root step like every other section here, not one per chown and hook. The step is
# a script root runs with `bash -s`, reaching the sandbox account through runuser (which never prompts) and printing one
# tab-separated verdict line per check, which the verdict loop turns into this script's results. No command under it may
# call sudo, and none may put sudo under `timeout`: timeout runs its command in a background process group,
# where a password read on the tty stops the job.
...
if ! grep -qx "${PROJ}" "${HOME}/.config/ai-tools/allowed-projects" 2>/dev/null; then
    ...
    # The fixture directory, named by the suite's fixture rule so a leftover reads as the suite's. 2770: the hook stats
    # the path AS the agent before delegating, and the sweeps walk it, so the sandbox group needs entry; setgid keeps
    # that group on everything born inside. The claim's default ACL keeps this operator's own access on every path
    # the hooks hand back.
...
ROOT_STEP
    ...
    # Whatever a failed hook left agent-owned, the unclaim in section 3 hands back; the operator's default ACL keeps
...
if [[ -z "${OTHER_OP}" ]]; then
    ...
    #
    # The shared sandbox area is the one place on a stock install that both operators reach: it carries g:ai-ops:rwX
    # plus a default ACL, applied at install, and is deliberately outside the protected-paths set. If this host has not
    # got it, the drill skips rather than inventing a
    # location.
    if [[ ! -d "${SANDBOX_ROOT}" ]]; then
        ...
        # Named by the suite's fixture rule, so a tree the removal did not complete is found by `tests/run.sh residue`,
        # which runs as root and can remove what this script cannot.
        ...
        if grep -qi 'holds no sudo grant to run' <<<"${FC_OUT}"; then
            ...
            # give every ai-tools helper and still restrict which accounts you may act as. It must fire BEFORE anything
            # is created.
...
# A stray copy of a claimed tree, at a path no allowlist names: the case --force exists for. It is built INSIDE
# the claimed project and then moved out, because an unprivileged `cp -a` cannot reproduce it -- the operator is
# deliberately not a member of the sandbox group, so it may not chgrp to it, and POSIX lets cp -p drop uid/gid silently
# when it may not set them. Staged inside, the copy inherits group and default ACL from the setgid parent; `mv` is
# a rename, so it carries both out intact. That is also how the real case arises: the tree is moved, not re-created.
...
# The hand-back itself, and the hardlink refusal that qualifies it. Both describe what an unclaim does to a GRANTED
# tree, so both are skipped rather than asserted against one that never was.
if ! ${TREE_CLAIMED}; then
    ...
    # The code is the refusal's identity (this script prints its own results, so it matches the code line the way
    # harness.sh's assert_msg does); the `find` line is content and stays.
    ...
    # The disclosed consequence: refusing leaves the inode in the agent's group, which is exactly why the message has
    # to say so. Assert the file the walk refused, not one it handed back.
...
# which is the worst outcome a check can have.
if [[ ! -d "${COPY}" ]]; then
...
# The timer's verdict must rest on a run SYSTEMD started, which the stamp's TRIGGER records. Driving both sides
# of that would mean rewriting the stamp -- shared runtime state a real update run owns -- so this reads the stamp
# the host already has and asserts the report AGREES with it. Whichever value is there, one direction of the rule gets
# exercised; the other is stated, and unit/services.sh drives both against its own fixture.
...
if [[ ! -r "${STAMP}" ]]; then
    ...
    case "${TRIGGER:-}" in
        unit)
            # A systemd-started run is evidence about the schedule; fresh it reads OK, past the 48h grace STALE. Both
            # are verdicts rather than '?', which is the property here.
        ...
        ""|manual)
            # A hand run, or a stamp predating the field, is no evidence about a schedule: the timer must decline rather
            # than read OK off someone else's run.
...
# The harness now takes the invoker as the project user, so these run without sudo. Only from a checkout;
# an installed-only host skips them.
...
# session_task_count: how many tasks the sandbox account's slice holds OUTSIDE the user manager's own init.scope -- i.e.
# how much agent work is running. Read from cgroupfs directly, which is world-readable, so this observes the same fact
# the helper acts on without going through it.
session_task_count() {
...
# manager_is_up: does the sandbox account's user manager hold any task? Read as CONTENT, and NEVER with `-s`: every
# cgroupfs file stats as zero bytes however many tasks it holds, so `[[ -s cgroup.procs ]]` is false even for a slice
# holding hundreds -- an assertion that cannot pass, and that reports a healthy restore as a failure. (Verified:
# the root cgroup.procs stats 0 with 379 pids in it.) session_task_count already reads content, for the same reason.
#
# `2>/dev/null` PRECEDES the input redirect, per the rule ai-tools-stop.sh's cgroup_pids states: redirections apply left
# to right, so the other order lets a missing init.scope -- exactly the case this asserts against -- write its open
# failure to the real stderr, mid-report.
manager_is_up() {
    local line
...
if [[ -z "${SANDBOX_UID_N}" || ! -d "${SANDBOX_SLICE_DIR}" ]]; then
    ...
    # Asserted by the absence of a prompt, which is why no sudo_why precedes it.
    ...
    if ! ${STOP_ALL_DRILL}; then
        ...
        # THE USER MANAGER IS TERMINATED TOO, AND PUT BACK. It is not spared: an exemption is a cgroup a session can
        # move into on a DAC-only host, so the sweep covers init.scope like everything else and the manager is restarted
        # afterwards. What matters to the next launch is only that it is back -- give it a moment, since the restart is
        # asynchronous.
        ...
        # IDEMPOTENT IN END STATE, WHICH IS NOT THE SAME AS SILENT -- and the difference follows from the rebuild rather
