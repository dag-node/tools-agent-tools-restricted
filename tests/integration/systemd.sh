...
# Integration: the shipped systemd units parse cleanly and are enabled in the right instance. `systemd-analyze verify`
# catches a directive typo that would otherwise ship silently; the enablement checks confirm the install wired each unit
# where it runs -- the toolchain timer in the sandbox account's own `--user instance`, the relabel watcher and handback
# socket in the system instance. Run as root.
...
# The `--user units` this project ships. Other packages install into the same directory, and this suite judges only its
# own units, so every check over the user units reads this one list.
readonly -a SHIPPED_USER_UNITS=(nvm-update.service nvm-update.timer)
SANDBOX_UID="$(id -u "${SANDBOX_USER}" 2>/dev/null || true)"

# sandbox_systemctl <args...>: run `systemctl --user` in the sandbox account's own instance. XDG_RUNTIME_DIR alone lets
# systemctl auto-discover D-Bus (EL9) or Varlink (EL10); forcing DBUS_SESSION_BUS_ADDRESS breaks EL10 where the bus
# socket may not exist.
sandbox_systemctl() {
    # Prefer the machine transport. This suite runs as root, and dropping into the account with sudo needs
    # that account's own bus to accept the connection -- which a host may refuse while the manager is perfectly healthy,
    # making a bus refusal indistinguishable from an absent manager. The machine transport reaches the same manager
    # over the system bus, where root is already authorized; the sudo form is the fallback for a systemd without it.
...
if ! command -v systemd-analyze >/dev/null 2>&1; then
    ...
    # nodocs, so `man restorecon(8)` is an environment gap, not a unit defect; any other complaint still FAILs.
    verify_judge() {
    ...
    # System units. The handback@.service template is instantiated at runtime (the socket passes the connection), so it
    # is not verifiable standalone and is left to handback.sh.
    ...
    # User units verify against the `--user manager` context, so run as the sandbox account with its runtime dir.
    # `systemd-analyze --user` as root has no XDG_RUNTIME_DIR and fails the RuntimeDirectory lookup -- that is
    # the caller's missing context, not a unit defect.
...
#
# It is a text check rather than a runtime one because the unit starts either way: `systemd-analyze verify` does not
# report the option, and RestrictNamespaces= does not prevent it (it filters the payload's own unshare/clone/setns,
# which systemd installs after building the namespace), so the unit file is where it is catchable.
#
# The check covers EVERY `--user unit` this project ships, not the one where this was found, because the property
# belongs to the manager rather than to the updater. A unit another package installs beside them is that package's
# to judge, so only SHIPPED_USER_UNITS is read.
...
for u in "${SHIPPED_USER_UNITS[@]}"; do
    ...
    # Assignments only, so a directive named in a comment (this rationale, or a unit header explaining the prohibition)
    # is not read as one being set.
...
# asserted on its own because the enable comes from the shipped preset (85-ai-tools.preset) -- a package that ships it
# disabled leaves the handback silently dead (the class of bug where a source install worked but the RPM did not),
# which this catches even if the socket happens to be started by hand.
...
# (2) Relabel watcher: enabled in the system instance, so a post-upgrade symlink repoint triggers the entrypoint relabel
...
# (3) Toolchain timer: active in the SANDBOX account's own `--user instance` (not the operator's), where the updater
# writes the shared .nvm tree directly. The timer is active only while that `--user manager` runs, and what keeps
# the manager running with no login is the account's linger, which `ai-tools-admin operators add` enables. This reads
# that state and does not repair it -- the suite does not start or stop a service on the host, and a manager it had
# started would either stay up as a change the run made or be stopped along with any session launched meanwhile.
# So: linger absent is a FAILURE (the enrolment did not take, and ai-tools-run aborts at the bus socket on such a host);
# linger present with the manager down is the container case, where logind does not sustain the lingering instance
# across the suite's session open/close, and the runtime state is untestable here -- skip with the start command named
# (the on-disk enablement check and `systemd-analyze verify` already cover correctness).
if [[ -z "${SANDBOX_UID}" ]]; then
    ...
    # The manager reaches timers.target (and starts the wants-linked timer) shortly after its bus comes up, so retry
    # briefly rather than reading the state in the same instant.
    _timer_active=""
...
if [[ ! -x "${CLI}" ]]; then
    ...
    cli_cmd ai-tools.status || exit 2
    out="$(runuser -u "${PROJECTS_USER}" -- env HOME="${PROJECTS_HOME}" "${CLI}" "${CLI_ARGV[@]}" 2>&1)" && rc=0 || rc=$?
