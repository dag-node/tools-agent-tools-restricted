...
#
#      Driven through the CLI as the operator against this hermetic HOME, so nothing here writes
#      the real registry. It reaches no root helper (the pair edits one line of the caller's own
#      allowlist), so there is no sudo and no password prompt.
cli=/usr/local/bin/ai-tools
if [[ ! -x "${cli}" ]]; then
    skip "disabled project refused at launch" "${cli} not installed"
else
    run_cli() {  # $@ = CLI args, run as the operator against the fixture HOME
