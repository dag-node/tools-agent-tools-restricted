...
# Integration: the single source of truth for deployed-artifact ownership/permissions, plus sudoers syntax. Asserts
# EVERY installed file and directory matches the security model -- root-owned helpers and handback bridge, the CLI,
# the locked /opt/ai-tools/bin and ai-tools-run, the setgid sandbox/control-plane dirs, the setgid+sticky .claude,
# agent-readable-but-not-writable hooks/config, the root-only operation logs, and the projects-user-only
# allowlist/config -- and that the sudoers drop-in parses. Needs a completed install; run as root via sudo,
# or via `sudo ./install.sh check-perms`.
...
# SELinux project-label helper: 750 root:root -- user-run via sudo, never by the agent (no SANDBOX_USER grant); same
# surface as lockdown.
check_file /usr/local/libexec/ai-tools/ai-tools-relabel          root              root              750
# SELinux agent-relabel helper: 750 root:root -- run AS root automatically by the ai-tools-relabel.path watcher
# and on demand by `sudo ai-tools-admin system entrypoints relabel`, never by the agent. It carries no NOPASSWD rule:
# the admin command reaches it through the host's own general sudo grant, so the %ai-ops drop-in holds the session
# lifecycle alone.
check_file /usr/local/libexec/ai-tools/ai-tools-relabel-agent    root              root              750
# Toolchain bootstrap + operator administration: 750 root:root. ai-tools-admin runs as root via sudo; the provisioning
# helper is exec'd by its `system bootstrap` command at this fixed path. Neither is reachable by the agent (no
# SANDBOX_USER grant, and /usr/local/libexec/ai-tools is 750 root:root).
...
# The sudo-PATH symlink in /usr/sbin, for the one command an administrator types (sudoers secure_path on stock EL
# excludes /usr/local/sbin, so `sudo ai-tools-admin` resolves here). check_file lstat()s the link itself (777 is
# a symlink's fixed mode); `-e` inside it also catches a dangling link. Nothing else has one: the provisioning helper
# and every contributed command are reached as verbs of this one.
check_file /usr/sbin/ai-tools-admin                           root              root              777
# Lib dir: root-owned, group ai-tools, 0751. The agent enters via group to read the skip list; world-execute lets
# an operator (not a SANDBOX_GROUP member) traverse in to source the 644 world-readable libs by path without listing
# the dir. No write but root.
check_file /usr/local/lib/ai-tools                            root              "${SANDBOX_GROUP}" 751
# Secret-pattern matcher: read only by the root helpers, so 640 root:root -- no group/world surface. The agent (group
# ai-tools) cannot read it.
check_file /usr/local/lib/ai-tools/secret-patterns.lib.sh     root              root              644
# Skip-dir list/selector: 644 root:root -- world-readable, sourced by the root helpers, session-hook.sh (runs
# as the agent), and the CLI's claim drift scan (runs as the projects user, not in ai-tools). Carries no secrets:
# the names are documented.
check_file /usr/local/lib/ai-tools/skip-dirs.lib.sh           root              root              644
# Logger library: 644 root:root -- world-readable, sourced by the root helpers, the hooks (run as ai-tools), and the CLI
# (run as the projects user, not in ai-tools).
check_file /usr/local/lib/ai-tools/log.lib.sh                 root              root              644
# Project-label library: 640 root:root -- read only by root principals (ai-tools-relabel and install-selinux.sh). No
# group/world surface; the unprivileged CLI inlines its read-only label check instead of sourcing it.
check_file /usr/local/lib/ai-tools/relabel.lib.sh             root              root              644
# Operator-identity resolver: 644 root:root -- world-readable like log.lib.sh; sourced by the root helpers
# (ai_tools_handback_t) and the agent hooks (ai_tools_t) to read operator.conf.
check_file /usr/local/lib/ai-tools/operator.lib.sh           root              root              644
# Control-plane boundary-mode constants: 644 root:root. Sourced by install.sh as the single source for the /opt/ai-tools
# home/dir modes the spec %files also declares.
check_file /usr/local/lib/ai-tools/control-plane.lib.sh      root              root              644
# Managed-asset seeder: 644 root:root. Sourced by install.sh + ai-tools-bootstrap to seed the ai-tools-* agents/skills;
...
# The compiled policy modules ai-tools-admin loads a stable group from: 644 root:root in a 755 root:root directory,
# and the SET is asserted with the modes -- exactly the modules selinux/policy/shipped-modules.sh derives
# from the registry and the integration manifests, which is the list the RPM build and install.sh compile. A module
# missing from it is a stable group with no module for `selinux groups enable` to load; one beyond it was not built
# by this release. The directory exists only where the SELinux layer was staged (the RPM ships it everywhere; a source
# install stages it on a host with SELinux active), so its absence is a skip, not a failure.
...
# Command-filter engine: 644 root:root -- world-readable, sourced by an agent's filter hook, which runs AS the agent
...
# or fragment inside it. The resolver and ai-tools-run both refuse a directory that fails exactly this check,
# so an assertion here is the deployed-state half of that guarantee (see providers.rule.md).
...
# The contributed-command directory takes that same reasoning at the highest privilege here: ai-tools-admin execs
# what it finds inside AS ROOT, so a non-root writer would be choosing a root command. Root-owned with no group or other
# write is what the dispatch's trust check requires; world-READ is what lets `ai-tools-admin --help`, answered ahead
# of its own root check, list this host's domains for any caller.
...
# dotnet integration data (shipped by ai-tools-integration-dotnet, not base): 644 root:root -- the manifest
# providers.lib.sh reads and the session-env fragment ai-tools-run sources when enabled.
...
# but a fresh RPM enrolment need not.
...
# Operator identity: 644 root:root -- world-readable (agent hooks + root helpers read it), root-write-only (the agent
# cannot rewrite the identity root hands files back to).
...
# readable by root and the sandbox account (which needs the token) but NOT world, and NOT by the operator (not
...
# /opt/ai-tools/bin is locked: root:ai-tools 0551, so ai-tools has group r-x but no write. The agent can execute
# nvm-update.sh and resolve the claude symlink, but cannot edit the updater or swap the symlink -- only root (via
# ai-tools-launcher-symlink) writes here. The o+x search bit lets an operator readlink bin/<launcher>.
check_file /opt/ai-tools/bin                                  root              "${SANDBOX_GROUP}" 551
# Control-plane files: root:ai-tools. The agent (running as ai-tools) gets group read/exec but no write, so it cannot
# rewrite its own updater, hook, or hook config.
...
# (control-plane.lib.sh), so a second agent is covered here without editing this list; a host with none skips and says
# so. The SHARED asset roots: base-owned, agent-readable, NOT agent-writable. Every agent symlinks into them,
# so a writable root here would let one session rewrite the instructions -- or the delegate definitions -- every agent
# and every future session reads.
...
# The orientation text itself, not only its root: it is what every session in every project loads before anything else,
# so an agent-writable copy would let one session rewrite what the next one is told about its own boundaries.
    ...
    for _spec in "skills_dir:/opt/ai-tools/skills:ai-tools-technical-docs" \
        ...
        while IFS=$'\t' read -r _agent _dir; do
            ...
            if [[ ! -e "${_link}" ]]; then
                ...
                # A real entry that is NOT ai-tools-managed is the operator's own override, which the linker is
                # contracted to leave alone -- the feature, not a regression.
...
# from the control-plane home), so the manager reads its units through the group but the agent cannot add
# a `--user unit`. An agent-writable wants dir would let a confined session register a unit the account's unconfined
# manager runs.
...
# Handback bridge. The helper dir is 750 root:root (no world bit -- non-root users cannot list the helper names).
# The daemon is root-only-executable; the agent never exec's it, it connects via the socket. The client is
# group-executable so SANDBOX_USER (a SANDBOX_GROUP member) runs it from the hooks/updater, but no world bit (no
# arbitrary user reaches the bridge). The units are read by systemd as root.
...
# Toolchain update units (sandbox account's `--user instance`) + post-upgrade relabel watcher. 644 root:root -- systemd
# reads them as root; no world write.
...
# Project CLI: 755 root:root -- runs AS the projects user (its guard refuses root and the sandbox account); root-owned
# so the agent cannot rewrite it, world-exec is harmless since it edits only user-writable registries.
check_file /usr/local/bin/ai-tools                            root root 755
# ai-tools(1) man page: the RPM's brp-compress gzips it, the from-source install does not, so check whichever form is
# deployed.
...
# Sandbox area: root:SANDBOX_GROUP. Outer dir 2750 (setgid, no world); inner sandbox-projects 2770 (setgid so clones are
# born group SANDBOX_GROUP, group-writable so the agent works in the clones). README 640.
...
# must be stripped symbolically, since neither `install -d -m` nor a numeric chmod clears it) -- and each stamp 0640
# SANDBOX_USER:ai-ops, the one inode the account rewrites in place, group-readable to operators and closed to everyone
# else. The stamp ships with the nodejs integration, so a base-only install legitimately lacks it.
...
# The entrypoint pins. root:root and not group-writable, unlike the stamp beside them: a stamp reports and does not gate
# a launch, while a pin is what the launch compares the agent binary against, so the account it constrains must not be
# able to write it.
...
# /opt/ai-tools root: 2751 root:SANDBOX_GROUP -- setgid propagates group SANDBOX_GROUP to new files; group r-x
# and the o+x search bit, so the agent reads through the group and an operator traverses to the launcher, but neither
# creates or deletes here. ai-tools-run is the base-owned confinement shim and mirrors nvm-update.sh (550, group r-x, no
# write): the sandbox account executes it, cannot modify it, and cannot replace it through the 0551 bin directory.
# .gitconfig 644: world-readable so the agent reads safe.directory and the operator/wrapper read it without
# SANDBOX_GROUP membership; only root writes (via ai-tools-safedir). .gitignore 640: a default-deny guard for a git repo
# versioning the control plane.
...
# Operation logs: dir 700 root:root, each file 600 root:root -- the root helpers append here; ai-tools (neither owner
# nor able to traverse the 700 dir) can neither read nor tamper with the trail, so secret filenames recorded
# by ai-tools-chown stay out of agent reach. The log FILES are %ghost (created on first write of their op), so each is
# optional: a fresh install has only the logs whose op has run (relabel.log waits for a relabel; install.log is
# install.sh-only).
...
# Projects-user config dir 700 + allowlist 600: ai-tools (not owner, not in PROJECTS_GROUP, cannot traverse the 700 dir)
# can neither read nor modify the approved-projects list even if it had a looser mode; the root helpers read it
...
# env_keep surface: ai-tools-run re-validates AI_TOOLS_AGENT_EXEC/AI_TOOLS_PROJECT_DIR (ai-tools-run.sh test), which is
# the real defense, but the drop-in's per-command env_keep should pass through ONLY those two -- a widened list would
# smuggle attacker-influenced env into the launch path. Pin it: every env_keep in the file names exactly
# AI_TOOLS_AGENT_EXEC and AI_TOOLS_PROJECT_DIR, and no other name.
if [[ -r /etc/sudoers.d/ai-tools ]]; then
    ...
    # The root rule must be pinned to its helper's ZERO-ARGUMENT form. The trailing "" is what keeps it a grant to run
    # one program one way: sudoers(5) reads a command listed with no arguments at all as permitting ANY, so a dropped ""
    # silently turns a narrow root rule into `--force` without a password. tests/unit/helper-path.sh pins the same line
    # in the SOURCE; this asserts what the install actually deployed.
    ...
    # And the drop-in holds the session lifecycle ALONE. The entrypoint relabel is an ai-tools-admin command reached
    # through the host's own general sudo grant, so a %ai-ops rule naming its helper would hand every operator
    # a passwordless root command the model no longer accounts for.
