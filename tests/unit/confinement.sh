...
# that ai-tools-run's fail-closed preflight dispatches on. Drives the truth table over the five probed inputs --
# getenforce, module presence, the matchpathcon-expected label, the live label, the manager domain -- and the operator's
# AI_TOOLS_REQUIRE_SELINUX switch, with no SELinux host required, so a regression in the gate (an inverted condition,
# a swallowed refusal) fails here rather than reaching production as an UNCONFINED launch. Sources the deployed library;
# does not need privilege of its own. Run as root via sudo (suite contract).
...
# expect <token> <rc> <enforce> <module> <want> <have> <mgrdom> [require]: drive the verdict and assert BOTH the echoed
# token and the 0=launch/1=refuse return. The '|| rc=$?' keeps a refusal (rc 1) non-fatal under `set -e` and captures
# the status. `require` defaults to unset (5-arg call), exercising that a 5-arg caller keeps today's DAC-capable
# behaviour.
expect() {
...
# ── The module-presence probe classifier (ai_tools_confinement_module_present) ── ai-tools-run derives the `module`
# verdict input from `matchpathcon` on a CORE-owned path, because it runs as the sandbox account and cannot read
# the root-only module store (`semodule -l`). A core-owned path resolves to an ai_tools_* type ONLY when the core
# module's file-contexts are live, so this classifier turns that probed type into the yes/no the verdict consumes.
# The false "no" this replaces was the fail-open: on the unresolved-label branch it would launch DAC-only
...
# ── AI_TOOLS_REQUIRE_SELINUX: the operator's opt-in that turns the two DAC-only LAUNCH exits into ── ── refusals. It
# ONLY tightens those two; every other verdict is identical to the require=no path. ──
...
# require does NOT loosen or alter any already-fail-closed refusal, and does not force a confined launch to refuse:
# a verified transition still launches, a mislabel/unverifiable still refuses.
...
# The manager-domain advisory stays advisory under require (it targets the /proc read, not the DAC-only launch exits):
# an unreadable ("") manager domain still launches.
