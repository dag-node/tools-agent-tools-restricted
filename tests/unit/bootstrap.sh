...
# Unit test for report_shadowed_operators -- the lines `ai-tools-admin system bootstrap` closes with
# when an enrolled operator's shell would run an agent of the launcher's name from somewhere other
# than /usr/local/bin.
#
# What gives it teeth is that this report is the last thing said before a host is treated as ready.
# A run that named nobody on a shadowed host would state readiness over an operator whose next
# `claude` starts an UNCONFINED session as them, and one that named an account on every host would
# be read past. So both directions are driven, and so is what the report tells the operator to do:
# the message names the account, the launcher and the binary that wins, and the two ways out are
# the enrolment command that ranks the wrapper first and the path to remove.
#
# The reading underneath it is path-order.lib.sh's and is pinned in unit/path-order.sh; what this
# file covers is the composition -- which operators are asked about, what is printed per record,
# that finding a fault does not become this command's exit status, and that no init file is written.
#
# The helper is SOURCED, not run: it stops at the guard before its provisioning, so one function is
# driven with no toolchain to install. Each case runs in its own bash, because the helper and the
# harness both declare SANDBOX_USER readonly. The libraries are sourced BEFORE the stubs, so the
# include guard makes the helper's own `source` a no-op and the stubs stand; the helper reads them
# at their installed paths, so a host without them skips rather than driving a different library.
...
# Where a second agent of the wrapper's name is found on a real host. This project's wrapper is
# /usr/local/bin/claude; what can win ahead of it is the operator's own `npm i -g` under their nvm,
# or the vendor package's /usr/bin/claude -- the same file as /bin/claude, which is the spelling the
# reading prints where /bin is the usr-merge symlink and PATH carries that form. Each is outside
# /usr/local/bin, so each shadows the wrapper, and the report names the one the shell would run.
...
# run_report <stub-code> -- drive one case and echo everything it said, with the function's own
# status on a last `rc=` line. The stubs land between the libraries and the helper, which is the
# one order in which they survive: sourced first they would be overwritten, and sourced after the
# helper they would not be in place when it runs.
run_report() {
    bash -c '
        set -euo pipefail
...
# The stubs a case composes from. `stub_reading <state> <users> <winner>` answers for the named
# accounts with that winner and leaves every other account reading as a host whose ordering is
# right, so a case states which accounts are shadowed and by which binary rather than restating the
# reading. Each recording that a reading was taken at all, which is what (F) asserts the absence of.
...
# The provisioning succeeded; what the report found is a state of the operator's shell, and a
# non-zero status here would report the toolchain install as failed.
...
# `operators add` is this project's one writer of the guard line, behind its confirm; a report that
# repointed on its own would edit an operator's home without asking.
...
# A host whose ordering is right, and one whose reading could not be taken, are the runs an
# operator sees most; a line on either teaches them to read past the one that matters.
...
# Bootstrap runs before the first enrolment as often as after it, and a reading is a login shell
# per account: with no operator recorded there is no account to read and none to name.
rm -f "${READ_MARKER}"
