...
# file is filled whole. Every path in `files` is relative to the repository root.
declare -A ranges=()
declare -a files=()
outside=0
...
# A named path is read from where the command was run and taken to the root, without resolving a
# symlink: the vetting in each filler refuses one, and a path it names must be the path it read.
for f in "${named[@]}"; do
...
# A file a filler refused is counted in the exit status and kept out of the closing report, which
# measures only what was read whole.
...
for f in "${files[@]}"; do
    ...
    case "${kind:-missing}" in
        ...
        source|header)
            # Emacs reports each file on stderr; a filler that could not run, or a file it refused,
...
# checker_report <finding-pattern> <checker-argument>...: the findings matching the pattern, or a
# failure where the checker could not run -- an exit other than 0 or 1 (its findings status), or
# anything on stderr.
checker_report() {
