...
# is reported, and exits 1 while any of that remains, while a filler refused a file, or where the
# checker itself could not run -- a report that read an aborted run as a clean one would exit 0
# over lines it never measured.
#
...
#
# What it formats is `FORMAT_SCOPE`, a list of path patterns this repository owns: a page, a source
# file's comment prose, a policy source's, a config header. A tracked file outside it -- a systemd
# unit, the spec, a Containerfile, a Makefile, a filter rules file, the sudoers drop-in, a
# gitignore, a licence text, a signing key, a lockfile, a capture or a log -- is data another
# program parses, and a fill would rewrap what that program reads, so every scope leaves it as
# written: `--all` and the diff scope count what they left, and a file named on the command line is
# reported and skipped. The checker's own kinds sit inside the scope: a man page, a binary and a
# generated file (one carrying the ignore-file marker) pass the pattern and are left by the
# dispatch. A pattern matches the path from the repository root, and `*` crosses a `/`.
#
...
#
# The body is one function, called on the last line: bash parses a function whole before running
# it, so this file is among the files `--all` fills. Read a command at a time, a script that is
# rewritten under a running bash is read on from the old offset, into the middle of a line.
set -euo pipefail
...
main() {
    ...
    # `ranges[<file>]` holds the added-line ranges of a touched file as `A-B,C-D`, or is unset where
    # the file is filled whole. Every path in `files` is relative to the repository root.
    local -A ranges=()
    ...
    # A named path is read from where the command was run and taken to the root, without resolving
    # a symlink: the vetting in each filler refuses one, and a path it names must be the path it
    # read.
    for f in "${named[@]}"; do
    ...
    # A file a filler refused is counted in the exit status and kept out of the closing report,
    # which measures only what was read whole.
    for f in "${files[@]}"; do
        header=()
        # shellcheck disable=SC2053  # the pattern side is the tool's own glob
        ...
        case "${kind:-missing}" in
            ...
            source|header)
                # Emacs reports each file on stderr; a filler that could not run, or a file it
                # refused, is reported there too.
    ...
    # The width findings alone, each with the line it names; `--wrap` also runs the default
    # checks, which are a writing matter and not the formatter's.

    # checker_report <finding-pattern> <checker-argument>...: the findings matching the pattern, or
    # a failure where the checker could not run -- an exit other than 0 or 1 (its findings status),
    # or anything on stderr.
    checker_report() {
