...
# tools/generators/man-messages.sh -- generate ai-tools-messages(7) from the cross-reference index. The page is derived, never
...
# ref-index.py.
#
#     bash tools/generators/man-messages.sh generate          rewrite the page from the index
#     bash tools/generators/man-messages.sh print [<index>]   write the page to stdout, from the index given
#                                                  (default: the committed one); `tests/unit/man.sh`
#                                                  renders a fixture catalog through it
#     bash tools/generators/man-messages.sh stale             exit 1 when the committed page differs from the index
#
...
# The page is excluded from the index's own inputs (tools/generators/ref-index.sh `files`), so listing a code here does not count
...
ai_tools_msg_error=ERROR die_stop_usage=ERROR reject_with_usage=ERROR coded_refusal=ERROR _pu_check_failed=ERROR
...
ai_tools_msg_warn=WARNING _ai_tools_toolchain_warn=WARNING _ai_tools_launch_warn=WARNING _pu_attention=WARNING
...
.\" GENERATED from .claude/references.md by tools/generators/man-messages.sh; do not edit this file. Change
.\" a message in the source that emits it and run `bash tools/generators/man-messages.sh generate`;
...
.\" Every message text is a runtime string copied from the index, so a writing finding here names
.\" prose this page cannot fix (the marker on the next line takes it out of the checker's report).
...
case "${command}" in
    ...
    stale)
        if ! render | diff -q - "${PAGE}" >/dev/null; then
            echo "${PAGE} is stale; run: bash tools/generators/man-messages.sh generate" >&2
            exit 1
        fi ;;
    *)
        echo "usage: bash tools/generators/man-messages.sh generate|print|stale" >&2
        exit 2 ;;
esac
