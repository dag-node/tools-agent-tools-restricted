...
#     bash tools/generators/ref-index.sh check             report every reference finding in the tree,
#                                               and every message string carrying a link
#     bash tools/generators/ref-index.sh messages          report a message string carrying a URL, a Markdown
#                              [<file>...]      link, or an HTML anchor; the tree, or the files given
#     bash tools/generators/ref-index.sh stale             exit 1 when the committed index differs from the tree
#     bash tools/generators/ref-index.sh relink            rewrite every reftag link in the tree's documents
#     bash tools/generators/ref-index.sh kinds             list the kinds and code families a reftag may name
#     bash tools/generators/ref-index.sh new <family>      mint a reftag whose id is unique across the tree,
#                              [--count N]      the retired file, and the wip tickets; N of them,
#                                               distinct from each other too
#     bash tools/generators/ref-index.sh where <reftag>    print the target's live file:line and span
#     ```
set -euo pipefail
...
case "${command}" in
    ...
    stale)
        if ! files | xargs python3 "${TOOL}" generate --at "${INDEX}" | diff -q - "${INDEX}" >/dev/null; then
            echo "${INDEX} is stale; run: bash tools/generators/ref-index.sh generate" >&2
            exit 1
        fi ;;
    ...
    *)
        echo "usage: bash tools/generators/ref-index.sh generate|check|messages [<file>...]|stale|relink|kinds|new <family>|where <reftag>" >&2
        exit 2 ;;
esac
