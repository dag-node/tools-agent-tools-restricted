...
7. The same `SessionStart` pass also normalizes the project's **setgid** bit, via
   ...
   project directory is set group `ai-tools` with `g+s`, so a file *you* create
   there is born in group `ai-tools` and the agent can read/write it — **without you
   being a member of the `ai-tools` group**. That lets the projects user stay out of
   the sandbox group entirely (defense in depth: your home-dir configs are then
   unreachable from the sandbox group) while collaboration on project files still
