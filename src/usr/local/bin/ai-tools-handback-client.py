...
#
# Connects to /run/ai-tools/handback.sock (AF_UNIX SOCK_STREAM), sends one "VERB ARG\n" request, reads the response,
# relays MSG lines to stderr (so the calling hook can surface NOTICEs in the agent's session), and exits 0 on OK or 1
# on ERR/error.
#
# This is how the agent process tree reaches ai-tools-{chown,setgid,launcher-symlink}. A `sudo` call cannot serve there:
# under NNP (PR_SET_NO_NEW_PRIVS, forced by
# RestrictNamespaces=yes in the session service unit) sudo loses its SUID bit and
# keeps running as @SANDBOX_USER@ -- unable to read /etc/sudoers or switch uid -- so the call fails silently.  This
# client uses no SUID at all: it connects a socket only root and @SANDBOX_GROUP@ members can reach (0660
...
def main():
    ...
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as sock:
            try:
                ...
                # Name the socket and the likely cause: a bare "[Errno 2] No such file or directory" from connect()
            ...
            # Read the response line by line.  sock.makefile() wraps the socket in a buffered text reader; closing it
            # does NOT close the underlying socket (Python documented behaviour), which is then closed
            # by the with block.
    ...
    # Fell through with no OK or ERR -- daemon closed the connection without a terminal response (crash, protocol
    # error).
