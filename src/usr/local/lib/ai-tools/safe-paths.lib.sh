...
# directory <owner_user> owns, and it either does not match any protected path or matches ONLY as
...
# `--x` entry on one directory -- search permission on that directory alone, which permits
# traversal and neither a listing of it nor any access to the files inside, whose own modes and
# ACLs still decide. Refusing
