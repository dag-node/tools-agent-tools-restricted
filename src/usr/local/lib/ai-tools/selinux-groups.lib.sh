...
readonly AI_TOOLS_SELINUX_GROUPS=(
    ...
    "netcore|.NET runtime IPC (dotnet test, multi-node MSBuild pipes) and executing files in the project tree (a built binary, and any script the tree carries)|The CoreCLR runtime opens diagnostic sockets and FIFOs under /tmp and .local that the base does not create there, and executes native hosts it built from the project tree (ai_tools_project_t) -- neither needed by the agent itself. The IPC half is benign and unblocks dotnet test and multi-node MSBuild; the execution half grants execute on every file labelled ai_tools_project_t -- a built binary, and equally a git hook or a project script -- so the module is off by default. Pairs with tmpmap and apphost; see .claude/rules/dotnet.rule.md.|experimental"
)
