...
readonly AI_TOOLS_SELINUX_GROUPS=(
    ...
    "memfdexec|Executing in-memory code the session generates itself (a .NET apphost/JIT: dotnet run, ASP.NET Core, worker services, xunit.v3, single-file; any W^X JIT emitting into a memfd)|A JIT writes generated native code to an anonymous memfd file and maps it a second time PROT_EXEC; the core grants execmem (anonymous RWX) but not execute on a FILE mapping, so any executable/host project fails to build or run. This gives the memfds the session creates a type of their own (ai_tools_memfd_t, through a tmpfs type_transition) and adds map+execute on that type, so the grant reaches no tmpfs file the session did not create. It permits fileless in-memory execution but grants no new privilege (still ai_tools_t, no entrypoint), and is separate from /tmp, which stays noexec. Library builds and in-process test runners (MSTest) do not need it.|stable"
