...
# Shared secret-name pattern set for the ai-tools sandbox. This file is *sourced* (never executed) by the root helpers
# ai-tools-chown and ai-tools-lockdown so both decide whether a basename is a credential file by the SAME rules
# from the SAME source -- the matcher cannot drift between them.
#
# The authoritative pattern list lives in a user-owned config file under the operator's home,
# `<PROJECTS_HOME>/.config/ai-tools/secret-patterns`, owned by the operator 600 (PROJECTS_HOME comes from the operator
# identity the caller resolves via operator.lib.sh). The user edits it; ai-tools -- neither its owner nor in its group,
...
# across software in general -- and does not hold any name specific to one deployment. A change belongs in one of two
# other places: a project- or organization-specific name goes in the operator's 600 config, and a name missing
# from the general baseline goes upstream as a pull request. The baseline is incomplete by construction, since it tracks
# conventions that keep appearing.
#
...
# written no patterns is silent.
#
# It exists because the operator's file REPLACES the baseline (secret-handling.rule.md) rather than extending it,
# which makes a stale copy silently NARROWER than what ships: a host that wrote its own file keeps quarantining what it
# listed then, and nothing added upstream since. That gap is invisible from every side -- the agent cannot read
# the file, and a classification that did not happen produces no output -- so the launch wrapper reports it once
# per session. The DROPPED patterns are the half that matters: each one is a credential name this host no longer
# quarantines.
#
