...
#
# The variables are those .NET 8 LTS and later read. DOTNET_CLI_HOME is what keeps the shared tools tree read-only:
# the SDK's own state defaults to $HOME/.dotnet, so it is pinned at the writable sibling instead.
#
# MSBUILDDISABLENODEREUSE=1 stops MSBuild from leaving persistent worker nodes running between
# invocations. Every operator's builds share one sandbox UID, so a reused node keeps a build task's assemblies loaded
# and locks the prior project's output file -- a second build in the same solution then fails on the lock
# (dotnet/msbuild#6461, for which the maintainers recommend exactly this variable). Disabling reuse costs one node start
# per build and does not change what a build
# produces.
#
...
# The host SDK/runtime tree at its RPM path, or wherever the muxer resolves to. The muxer is already on the session
# PATH, so PATH itself does not need a dotnet entry.
