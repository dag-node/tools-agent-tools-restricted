...
# Normalizes group ownership and the setgid bit on the directories of an approved project, so that files EITHER
# an operator OR the agent creates there are born in group @SANDBOX_GROUP@ -- the shared group both can read/write.
# setgid carries that group onto new files regardless of the creator's own group membership, which is what lets
...
# in the operator's sudo context. The agent that triggers it cannot read the allowlist, so the project path it passes is
# UNTRUSTED and re-validated here against the same allow/exclude rules.
#
# Owner-only directories are the exception: the operator sealed them, so they are left out of the agent's group,
# the sandbox residue they carry is stripped, and their subtree is skipped with them -- see owner-only.lib.sh,
# which `ai-tools-{setfacl,lockdown,chown}` share.
#
...
# Every disclosure this helper prints goes through warn, so the component prefix is stated once here instead of at each
# site. A leading message code (msg.lib.sh states the form) is printed on its own line ahead of the message, the shape
# tests/lib/harness.sh's assert_msg reads. Matched inline, since this helper reports before msg.lib.sh is loaded. Its
# one refusal exits 3 at its own site, so there is no status for a die() to carry. The code it printed is left
# in _warn_code, so a site that also RECORDS the situation passes the variable and the code literal stays at the emit
...
# into the agent's group -- and COUNTED, so a walk that normalized no directory is reported rather than silent;
# the project root hitting the guard is called out on its own, since it means the whole claim granted no access. Matched
# by numeric UID; PROJECTS_UID is the resolved operator (set by the owner resolution).
SANDBOX_UID="$(id -u "@SANDBOX_USER@" 2>/dev/null || echo -1)"
readonly SANDBOX_UID

# Shared leveled logger: journald (always) + the root-only file /var/log/ai-tools/setgid.log. Best-effort -- a no-op
...
# and ai-tools-lockdown). A missing lib (broken install) leaves a stub that descends everywhere -- a slower but correct
# walk.
...
# semantics as ai-tools-chown / ai-tools-lockdown, which read the allowlist through the same conf.lib.sh grammar.
_is_excluded() {
    local path="$1" pat
...
_safe_setgid() {
    ...
    # Owner guard: only the projects user's or the sandbox account's own dirs are eligible (re-verified TOCTOU-safe
    # on the pinned inode); skip anything else. Return 3, not 1, so the walk can tell a third-party owner from a stat
    # failure and report it. Without that split, a walk whose every directory was foreign-owned reports the same as one
    # that had no directory to touch.
    ...
    # Group and mode come from the pinned inode, so the seal decision and the strip act on the same directory
    # the mutation would. A sealed dir is left out of the agent's group and its residue stripped (owner-only.lib.sh);
    # returning 2 tells the walk to skip its subtree too.
    ...
    | { declare -a skip=()
        ...
        while IFS= read -r -d '' d; do
            ...
            if (( rc == 2 )); then
                ...
                # under it inherits neither, so the agent cannot enter the tree. Called out separately from the count
                # for that reason.
        ...
        # while every other step succeeds -- an operator-owned tree claimed for a different operator hits it on every
        # directory. A count on stderr is what turns that from an invisible no-op into something the claim can report.
        if (( thirdparty )); then
