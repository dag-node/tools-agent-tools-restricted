...
# Restores operator:ai-tools ownership on files and directories an agent session created or overwrote. Invoked as root
# by the ai-tools-handback daemon (ai_tools_handback_t) when a session's hooks, or ai-tools-run's session-end sweep
# for an agent that declares none, send a CHOWN request over the handback socket. Accepts a single regular-file
# or directory target; for directories it strips world bits while preserving group rwx so the agent can keep working
# in a dir it created. An interactive invocation confirms per path; `--yes` skips that for a batch caller
# (ai-tools-reclaim) that already confirmed its whole set.
#
# Reads the operator's allowed-projects allowlist for allow and exclude rules (its path is derived from the operator
# identity in /etc/ai-tools/operator.conf). That file is owned by the operator 600 -- root reads it here on ai-tools'
# behalf.
#
...
# Every refusal and the one NOTICE this helper emits print through this pair, so the component prefix is stated once
...
# before msg.lib.sh is loaded. The printed text is left in _warn_text, and its code in _warn_code, for a site that also
# records the situation through log.lib.sh: the log call passes the variable, so the code literal stays at the emit call
# the reference index reads as its definition
# (messaging.rule.md).
...
die() { warn "$@"; exit 1; }
# die_unsourced <lib> -- one situation for every library this helper requires, as in install.sh: the remedy (reinstall
# the libraries) is the same whichever one is missing.
...
# a batch caller (ai-tools-reclaim) that already took ONE confirmation for the whole set passes it so a long walk does
# not re-ask per path. The remaining argument is the path.
ASSUME_YES=false
TARGET=""
for arg in "$@"; do
    case "${arg}" in
...
# and the strip alike). Required and fail-closed like safe-paths.lib.sh: an unusable library must not leave
...
# _notify_secret: emit a one-line NOTICE that a secret-named file was written and ai-tools' read access revoked,
# to stderr (the PostToolUse hook relays it into the session) and -- at WARNING level -- to journald + the root-owned
# chown.log. log.lib.sh wraps each sink in `|| true`, so a sink that cannot be written never blocks the NOTICE.
...
# This run acts for one operator, so that operator rides as per-run log context (logging.rule.md). The project joins it
# once a path matches an allowlist entry.
...
if [[ "${#allowed[@]}" -gt 0 ]]; then
    for dir in "${allowed[@]}"; do
        if [[ "${canonical}" == "${dir}" || "${canonical}" == "${dir}/"* ]]; then
            ...
            # lstat (the GNU stat default), so a symlink is seen as itself and refused along with the devices. A regular
            # file must have nlink 1: a freshly written file has one link, and a hardlink could point at a sensitive
            # file outside the tree. A directory legitimately has nlink >= 2 (its own '.' plus each child's '..'),
            # so that guard is for files.
            ...
            # A directory is the agent's own workspace rather than a secret to revoke: taking away access to a dir it
            # must keep writing into would break it.
            ...
            # on OWNER-execute -- the only exec bit git records. What each target hands back and why is
            # in ownership-and-hooks.rule.md (directories and ordinary files) and secret-handling.rule.md (secrets);
            ...
            # Interactive invocation (terminal available): show changes and confirm. Non-interactive (hook context,
            # stdin is a pipe): apply silently -- the allowlist is the user's standing authorisation. `--yes` skips
            # the prompt for a batch caller that already confirmed the whole set.
            if ! ${ASSUME_YES} \
            ...
            # directory and can unlink and recreate this path -- as a symlink, a hardlink, or a different file -- at any
            # instant. chmod has no `--no-dereference`, so a symlink swapped in before it would let root chmod
            # an arbitrary file (e.g. /etc/shadow).
            #
            # Pin the inode with an open fd and act through /proc/self/fd: a held fd cannot be redirected by a later
            # path swap. open() does follow a symlink swapped in just before it, so after opening we re-verify the fd
            # resolves to the SAME inode already validated, still a regular file, still link count 1. Any mismatch means
            # a race -- bail. NB: brace-group the redirection. A bare `exec {fd}< file 2>/dev/null` applies 2>/dev/null
            ...
            # A quarantined secret is owner-only now, so strip the residue the mode only masks: a file born in a claimed
            # tree carries the project's inherited group ACL entry, and `go=` leaves it in place, dormant
            # (owner-only.lib.sh). Ordinary files keep theirs -- the agent is meant to go on co-writing those. Every
            # value handed to the strip is read from the PINNED inode, ${got_ftype} included: the strip acts
            # through that descriptor, so what describes it comes from it. The pre-open ${ftype} is a second read
            # of a path that may since have been swapped, which the pinned-fd type check keeps equal to this one.
            if ${is_secret} \
            ...
            # file or directory handback is routine bookkeeping (INFO). Both name the path, owner change and mode
            # change.
            if ${is_secret}; then
