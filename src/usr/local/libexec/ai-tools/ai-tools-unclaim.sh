...
# Every refusal and every disclosure this helper prints goes through this pair, so the component prefix is stated once
# here instead of at each site. A leading message code (msg.lib.sh states the form) is printed on its own line ahead
# of the message, the shape tests/lib/harness.sh's assert_msg reads. Matched inline, since this helper reports
# before msg.lib.sh is loaded. The code it printed is left in _warn_code, for a site that also records the situation
...
# `--unlisted`: act on a tree that is NOT in any allowed-projects (a claimed project copied or moved elsewhere and never
# unclaimed). It swaps one gate for another rather than removing one: the allowlist-membership check is skipped,
# and every path must instead carry the ai-tools residue fingerprint (_is_residue) to be touched at all.
# The protected-paths backstop, the owner guard, the hardlink guard, and the secret/'!' skips all still apply,
# so the mode is strictly NARROWER per path than a listed unclaim and identical in what it does to a path it accepts.
# `--full` additionally walks the skip-listed heavy trees (node_modules, .venv, caches), where residue would otherwise
# survive a copy.
UNLISTED=false
FULL=false
for _arg in "$@"; do
...
# The sandbox GID is the residue fingerprint's cheapest arm: a claim chgrp's the tree to it, and every copy method
# that carries residue at all (`cp -a`, `rsync -a`, `mv`, `tar -p`) preserves it.
...
# directory -- a slower but correct walk.
...
if ${UNLISTED}; then
    ...
    # instead the operator who INVOKED sudo, validated against OPERATORS in operator.conf: operator A can never rewrite
    # operator B's files, and a direct root call with no sudo context is refused rather than defaulting to some
    # operator. Fails closed at every step -- an unresolvable caller, an unconfigured one, or an unloadable operator.lib
    # (whose functions are then undefined, so the `||` fires) all stop the pass before any mutation.
    caller_uid="${SUDO_UID:-}"
    [[ -n "${caller_uid}" ]] \
...
# Refuse a '!'-excluded target outright, and in the default mode a target outside every registered project: unclaim must
# never modify permissions outside allowed-projects. Same gate as `ai-tools-{setgid,setfacl}` (a silent no-op
# on a foreign target). The management CLI runs the hand-back BEFORE it drops the allowlist entry, so a legitimate
# unclaim still resolves its owner and stays listed here. `--unlisted` swaps this whole-tree gate for the per-path
# residue gate in _safe_unclaim; it never runs with neither.
_is_excluded "${canonical}" && exit 0
if ${UNLISTED}; then
    # `--unlisted` is for a tree NO allowlist names. If the caller's own allowlist does name it, the caller chose
    # the wrong mode: refuse rather than run the narrower per-path gate over a registered project, where the full walk
...
# _is_residue <fd> <uid> <gid>: 0 when the pinned inode still carries the ai-tools fingerprint -- owned by the sandbox
# account, grouped to it, or carrying a named ACL entry for it. The UNION of the three is what makes the gate safe
# to run outside the allowlist: a partially handed-back tree can retain any one of them alone, and testing only
# the group would leave an ai-tools-OWNED file untouched, where the agent keeps access through the user bits. Read
# from the pinned fd, so it describes the same inode the mutation acts on. The uid and gid checks read values the caller
...
_safe_unclaim() {
    ...
    case "${got_ftype}" in
        ...
        "regular file"|"regular empty file")
            # Hardlink guard, the same boundary ai-tools-chown enforces: an inode with more than one name can be reached
            # from OUTSIDE this tree, and chgrp/chmod act on the inode, so acting here would change a path the walk
            # never authorized. Directories legitimately have nlink >= 2 ('.' plus each child's '..'), so this applies
            # to regular files only.
            if [[ "${got_nlink}" -ne 1 ]]; then exec {fd}<&-; return 2; fi
            ;;
        *) exec {fd}<&-; return 1 ;;            # never touch symlinks/fifos/devices
    esac
    # Residue gate (`--unlisted` only): outside the allowlist the tree does not carry authorization of its own,
    # so a path is touched ONLY while it still bears the ai-tools fingerprint. A path that never belonged to a claim is
    # left byte-for-byte as it is.
    if ${UNLISTED} && ! _is_residue "${fd}" "${got_uid}" "${got_gid}"; then
        exec {fd}<&-; return 1
    fi
    local rc=0
    # Order: clear ACL (incl. default) -> regroup -> drop group write, so chmod acts on clean mode bits and no masked
    # ACL grant survives. On directories also clear the setgid bit (added by claim) so the tree returns to plain perms
    # (770 -> 750); a numeric chmod cannot clear a directory's setgid, only the symbolic g-s can. Files keep their
    # setuid/setgid bits untouched (an sgid binary must not be silently altered); only group write is removed there (660
    # -> 640).
    setfacl -b   "/proc/self/fd/${fd}" 2>/dev/null || rc=1
    chgrp -- "${TARGET_GROUP}" "/proc/self/fd/${fd}" 2>/dev/null || rc=1
    # `--unlisted` only: hand a sandbox-OWNED inode back to the invoking operator. Regrouping alone would leave
    # the agent its access through the USER bits, and the ownership sweep that covers this for a registered project
    # (ai-tools-reclaim) refuses an unlisted path, so this pass is the only one that can reach it. A path the operator
    # already owns is left alone -- this never changes ownership away from a third party, which the owner guard has
    # already excluded.
    ...
    if [[ "${got_ftype}" == "directory" ]]; then
        ...
        # Drop group WRITE; also drop a stray group EXECUTE on a data file. `setfacl -b` promoted the ACL's group:: base
        # (r-x on a tree the agent wrote) into the mode, so a data file can land group-executable (0650); strip
        # that, keyed on OWNER-execute (the bit git records) so a genuine script (owner rwx) keeps group r-x (-> 750)
        # while a data file (owner rw) drops to 640. Relative g-w[,g-x] leaves any setuid/setgid bit on the file
        # untouched (an sgid binary is not altered).
        local fmode gxarg=""
...
# Walk the project's directories and files (one filesystem; heavy trees skipped unless `--full`). A '!'-excluded
# or secret-named directory has its whole subtree skipped; an excluded or secret regular file is skipped on its own.
# find runs WITHOUT `-L`, so a symlink is listed but never descended: a symlink loop inside the tree is unreachable
# by construction and needs neither cycle detection, and `-xdev` keeps the walk off other filesystems and bind mounts.
if ${FULL}; then
...
find "${expr[@]}" 2>/dev/null \
    | { declare -a skip=()
        ...
        # that keeps the group it has -- so after the project is deregistered those inodes still carry the agent's
        # group, which is the one thing an unclaim is for. The refusal is still right (the inode is reachable
        # from outside the tree, and this pass does not authorize a change out there), so what the operator needs is
        # to be told plainly and handed the command that lists them, not a silent difference between counts.
        if (( hardlinked )); then
...
# .git reversal: the main walk skips .git (the shared heavy-tree list), but a claim grouped it to @SANDBOX_GROUP@ (the
# recursive chgrp) and may have normalized it (ai-tools-setfacl `--with-git`: setgid + ACL), so a full unclaim must
# revert .git too -- otherwise the agent keeps git-history access through the group owner and the named ACL entry.
# Revert it here in one pass with the same per-entry reversal (clear ACL, regroup to <target-group>, drop group write,
# clear dir setgid) and the same secret/exclusion skips. Unconditional: it reverses the base claim's chgrp whether
# or not `--with-git` ran, and no-ops on an already- reverted tree. The loop runs in this shell (process substitution),
# so the counter survives.
...
if [[ -d "${gitdir}" ]] && ! _is_excluded "${gitdir}"; then
    ...
    # the hardlink guard here in bulk. Refusing is the correct outcome -- those inodes are shared with the origin,
    # and changing one changes the origin's copy -- but it has to be said out loud, or the operator reads a partial
    # revert as a complete one.
...
# SELinux label reset (`--unlisted` only): a tree that was MOVED rather than copied keeps the ai_tools_project_t label
# it was claimed with. No fcontext rule names the new path, so `ai-tools-relabel --remove` has no rule to remove;
# a forced restorecon resets the tree to the default its location resolves to. Gated on the root actually carrying
# the label, so a tree that never had it is not relabelled as a side effect of unclaiming. Best-effort: a label left
# behind is a defence-in-depth gap, not an access grant -- the DAC reversal has already removed the agent's reach.
