...
You run as the `ai-tools` service account, not as the operator who launched you. The reduced privileges are intentional,
not a broken environment. These are boundaries sessions have repeatedly misdiagnosed.
...
- Any project claimed for this operator is reachable by exact path, not only the one you were launched in. Project
  parents, and the operator's home caches and dotfiles, cannot be listed.
- Credential-pattern files (`.env`, `*.pem`, `*.key`, `id_rsa`, and similar) are operator-only, including inside
  projects. Permission errors from `find`/`grep` on them are expected.
- Denial logs (journald, the audit log, `/var/log/ai-tools`) are unreadable. Surface the blocked operation instead
  of investigating the denial.
