...
- **NAME** — `name \- one-line description`, all lowercase apart from proper nouns and
  ...
  than a slogan.
- **SYNOPSIS** — the grammar, in the fonts and the placeholder vocabulary
  [Placeholders](#placeholders) fixes: `.B name`, then `.I COMMAND`, then the option and argument
  groups, each on its own line.
...
A replaceable argument is written **uppercase**, in italic, and named for what it is. The
vocabulary is the one `man-pages(7)` and the coreutils pages share, so a reader meets the same
word on this project's pages as on `cp(1)`:
...
A page names its argument by what the parser takes: a project directory is a `DIRECTORY`, not a
`PATH`. Where a directory argument defaults to the current one, `.` names it by
`path_resolution(7)`, so the DESCRIPTION states the default and the page does not document `.`
separately.

**Repetition is `...` after the placeholder, and only where the parser accepts several** —
`GROUP...` for one or more, `[FILE]...` for zero or more. A verb taking one argument does not
carry an ellipsis: the SYNOPSIS documents the parser as it is, not a surface the tool might grow.
`[]` on its own marks an optional argument, and `|` separates alternatives.

A pointy-bracket placeholder (`<path>`, `<dir>`) and a lowercase one (`[path]`) are off style on a
page: roff renders the brackets literally, and lowercase is the font's signal for something else.

**This is a convention for pages.** A shell function's signature, a `usage()` heredoc, a comment
and a runtime message each have their own reader and their own grammar, and they keep `<dir>`
where that is what they already write. A page generated from runtime strings —
this project's `ai-tools-messages(7)` — shows each message as it is emitted, so the rule does not
reach it either.

## Fonts, and where the rule binds

Bold is what the reader types as shown: the command name, a fixed option, a literal value. Italic
is what they substitute. Mixed forms take `.BI`, `.BR`, `.IR`, `.RB`, `.RI`; a raw `\f` escape is
for the inside of one macro's argument, where a high-level macro cannot scope it.

The strict reading of that — **every italic token is an uppercase placeholder** — binds on a
SYNOPSIS line and on the tag line opening a `.TP` or `.TQ`, which is where a reader looks for the
grammar. In running text italic keeps its other jobs: `man-pages(7)` sets filenames and
expressions in italic, and emphasis is italic in any prose. A filename is italic wherever it
appears, tag line included.

A literal value belongs in bold on both surfaces. A tag therefore reads
`.BR --scope " " minimal | full`, which keeps italic for what the reader replaces.
...
- **Fonts and placeholders** are covered by [Placeholders](#placeholders) and
  [Fonts, and where the rule binds](#fonts-and-where-the-rule-binds).
...
In this repository the first of those, the placeholder rule and the font rule run together over
every authored page as part of `tests/unit/man.sh`, which also holds each page's trailing argument
to the optionality and repetition its command's own `usage()` states.
