...
# A cell splits on an UNESCAPED pipe. A name may hold one -- a message naming an option set reads
# `--scope minimal|full` -- and a raw pipe there would open a column, so it is written `\|` and
# read back plain. A row therefore carries one cell per column whatever its values say.
CELL = re.compile(r"(?<!\\)\|")
MARKDOWN = (".md",)
...
emitter column carries, for a message code, the word that emits it. A pipe inside a name is
written `\\|`, so a row splits into its columns on the unescaped pipes alone.
...
def unescape_cell(value):
    """Return the cell's value, with the row's pipe escaping removed."""
    return value.replace(r"\|", "|")
...
def read_index(path):
    ...
    for row in map(INDEX_ROW.match, lines):
        if row:
            cells = [unescape_cell(part.strip()) for part in CELL.split(row.group(4))]
...
def command_generate(args):
    ...
    for target in sorted(targets.values(), key=lambda t: (t.path, t.line)):
        ...
        rows.append(f"| {id_of(target.reftag)} | {shown} | {escape_cell(target.name)} | {target.path} "
...
def command_retire(args):
    ...
    for reftag, cells in entries.items():
        if reftag in targets or reftag in retired:
            continue
        # The index row's cells after the id, restored to the index's column count and re-escaped
        # for the row they are written into; a row from an older index without the emitter column
        # takes an empty one.
        body = [escape_cell(value) for value in cells[1:]] + [""] * (4 - len(cells[1:]))
