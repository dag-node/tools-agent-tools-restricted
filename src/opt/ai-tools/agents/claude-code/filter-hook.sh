...
# Claude Code's adapter onto the shared token-saving filters (filters.lib.sh). Two events, one script, dispatched on $1
# the way session-hook.sh dispatches its session phases:
#
...
#
# Only the JSON shapes are Claude Code's; every decision is made in filters.lib.sh, so a second agent reuses the rules
# with an adapter of its own.
#
# A rewrite is NOT a permission decision. This hook never returns permissionDecision, so the harness runs its full
# permission pipeline on the result: an allow rule must still match it and a deny rule still overrides. That is
# what keeps a rules file from being a way around settings.json rather than an input to it.
#
# Fail SOFT, deliberately -- the opposite of the fail-closed rule the security gates follow. Filtering is a pure output
# path: a missing library, absent jq, a malformed event, a rule that does not apply all end the same way, with this hook
# emitting no output and the command running exactly as the agent wrote it. There is no state in which failing here
...
# Its domain rule is filters.rule.md.
...
case "${event}" in
pre-tool-use)
    # One jq pass covers both the tool check and the extraction; a non-Bash event yields an empty result. The matcher
    # in settings.json already scopes this to Bash -- the check does not add a fork and keeps the script correct
...
post-tool-use)
    ...
    # Command substitution strips trailing newlines, so every capture here carries an `x` sentinel appended inside
    # the substitution and removed after: a filtered value must differ from the raw one only by the noise removed, never
    # by a lost final newline. `jq -j` prints the string without adding a newline of its own, which is what makes
    # the raw capture exact.
