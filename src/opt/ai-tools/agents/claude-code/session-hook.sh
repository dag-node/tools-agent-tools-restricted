...
# Sandbox housekeeping hook, run at lifecycle boundaries (Stop + SessionStart). It hands back every ai-tools-owned file
# and directory under the session's project that the precise Write|Edit PostToolUse hook did not catch -- chiefly files
# created/modified via the Bash tool (npm/build output, codegen, sed/mv, redirects), which carry no file_path
# and so never trigger post-tool-hook.sh -- and, at session start, normalizes the project's setgid bit (via
# ai-tools-setgid) so files the projects user creates inherit @SANDBOX_GROUP@.
#
# Runs as ai-tools. Reads the hook JSON on stdin for .cwd (the allowlisted project root claude launched in) and sweeps
# there. Each path is handed to the root validator ai-tools-chown (via the handback socket bridge), which independently
# re-checks the allowlist and the agent-owned guard -- so this sweep cannot reach a path the precise hook could not.
#
...
# tool -> no Write|Edit PostToolUse) are never handed back by the sweep, on a graceful exit as much as a killed one --
...
# the sweeps out of the agent's control (see ownership-and-hooks.rule.md). Wired to the Stop, SessionStart,
# and SessionEnd hooks in settings.json (the SessionStart entry passes "session-start", the SessionEnd entry
# "session-end").

set -euo pipefail

# The hook's own directory -- this agent's config dir, whose name its manifest declares -- so the state files follow
# the hook instead of repeating a path the base layer no longer owns.
...
# files; the sudo helpers it calls record the actual file mutations there). Best-effort no-op fallback if the lib is
# missing.
...
# THE one msg.lib consumer that keeps a fallback: every other consumer requires the lib (it carries their yes/no
# decisions and their runs may fail closed), but this hook only EMITS, and its sweep is itself the safety action --
# the handback must run even on an install broken enough to lose the formatter, so a missing lib degrades the notice
# to plain text instead of stopping the sweep (see messaging.rule.md).
...
# so it cannot corrupt this function's captured count or the additionalContext JSON this script emits. No PROJECT/.git
# -> echo 0. Used by the session-end reclaim and the session-start pass.
reclaim_git_tree() {
...
# session-end: graceful process exit. Clear the clean-exit marker so the next session-start does not read this session
# as interrupted, and reclaim this project's .git now that the session is over and no live git command is there
# to disturb. Ownership then tracks the work tree the Stop sweeps already handed back, instead of waiting for the next
...
# the agent writes there via `git commit` (Bash tool, no file_path, so no Write|Edit PostToolUse handback) escape
# the sweep and leave .git in mixed ownership -- work tree <you>-owned, .git internals ai-tools-owned -- which makes git
# report "dubious ownership" and, once <you> is not an ai-tools group member, blocks reads and repacks. The marker does
# not gate this reclaim; it only selects the cross-project target and the NOTICE wording.
if [[ "${unbounded}" -eq 1 ]]; then
  if [[ -S "${HANDBACK_SOCKET}" ]]; then
    ...
    # the INTERRUPTED case is ALSO surfaced as SessionStart additionalContext, because only it is actionable: a killed
    # prior session can leave cross-project mixed ownership the agent should relay, with the manual reconcile
    # for stragglers the helper could not reach (excluded or quarantined paths). The routine post-git-activity reclaim
    # has already repaired ownership, so its notice would carry a line the user cannot act on, and injecting
    # additionalContext would force a TUI re-render that clobbers the agent's startup banner. It therefore stays
    # journald-only.
