...
path*, which is the same rule that keeps the `dotnet` admin command out: its targets are fixed
literals compiled into it, so it has no caller-supplied path to vet.
...
- **Deployed `644 root:root`**, world-readable like `msg.lib.sh`/`log.lib.sh`: the operator
  ...
  guarantee hold for that operator. Every library in that directory takes the same `644`, since
  each carries shipped logic or a general list; root-only write is what protects them, not the
  read bit (see [secret-handling](secret-handling.rule.md)).
